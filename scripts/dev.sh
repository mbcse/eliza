...
if [ -d "$CORE_PACKAGE" ]; then
  COMMANDS+=("npm --dir $CORE_PACKAGE dev -- $*")
else
...
if [ -d "./agent" ]; then
  ...
  COMMANDS+=("nodemon ${WATCH_PATHS[@]} -e js,json,map --delay 2 --exec 'npm --dir agent dev -- $*'")
else
