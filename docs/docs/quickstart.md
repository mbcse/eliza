...
- [npm 9+](https://npm.io/installation)
...
**Important Note on npm Lockfile Management**

By default, the `npm` lockfile will not be updated during installations based off of .npmrc frozen-lockfile=true. To update the lockfile, you need to run the command:
...
2. **Start the Agent**
    ...
    ```bash
    npm start --character="characters/trump.character.json"
    ```

    You can also load multiple characters with the characters option with a comma separated list:

    ```bash
    npm start --characters="characters/trump.character.json,characters/tate.character.json"
    ```
...
4. **Exit Status 1**
   If you see
    ...
    npm add -w -D ts-node typescript @types/node

    # Add dependencies to the agent package specifically
    npm add -D ts-node typescript @types/node --filter "@ai16z/agent"

    # Also add to the core package since it's needed there too
    npm add -D ts-node typescript @types/node --filter "@ai16z/eliza"
...
5. **Better sqlite3 was compiled against a different Node.js version**
   If you see
    ...
    ```bash
    rm -fr node_modules; npm store prune
    ```
